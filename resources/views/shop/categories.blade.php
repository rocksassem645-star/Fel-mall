...
<div class="container py-5">
    <h1 class="mb-4 text-center">{{ __('language.Shop by Category') }}</h1>
    ...
    <div class="row g-4">
                ...
                <a href="{{ route('category.products', $category->id) }}" class="text-decoration-none">
                    <div class="card shadow-sm border-0 rounded-4 text-center h-100 hover-lift">
                        <div class="card-body">
                            ...
                            <p class="text-muted small">{{ $category->products_count ?? 0 }} {{ __('language.products') }}</p>
                        </div>
                    </div>
                </a>
            </div>
        @empty
            <div class="col-12">
                <div class="alert alert-info text-center">{{ __('language.No categories found') }}</div>
            </div>
        @endforelse
    </div>
</div>
