...
<div class="container py-5">
    ...
    @else
                        ...
                        <tr>
                            ...
                            <td>
                                <form action="{{ route('cart.update', $item['product']->id) }}" method="POST" class="d-inline">
                                    ...
                                    <button type="submit" class="btn btn-sm btn-secondary">{{ __('language.Update') }}</button>
                                </form>
                            </td>
                            ...
                            <td>
                                <form action="{{ route('cart.remove', $item['product']->id) }}" method="POST">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="btn btn-sm btn-danger">{{ __('language.Remove') }}</button>
                                </form>
                            </td>
                        </tr>
                    @endforeach
        ...
        <div class="d-flex justify-content-between mt-4">
            ...
            <a href="{{ route('checkout') }}" class="btn btn-success btn-lg">{{ __('language.Proceed to Checkout') }}</a>
        </div>
    @endif
</div>
@endsection